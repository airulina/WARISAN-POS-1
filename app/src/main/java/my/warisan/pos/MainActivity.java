  ...
  void applyCorrection20260926(){
    ...
    try{
      ...
      android.content.SharedPreferences.Editor ed=p.edit().putString("stock_entries",keep.toString())
        .putInt("supplier_due",17500).putInt("supplier_due_sate",17500)
...
googleSignInClient = GoogleSignIn.getClient(this, gso);snapshotBeforeUpdate();loadMenu();clearCurrentNasiDamageOnce();applyCorrection20260926();selectedDay=date();selectedMonth=selectedDay.substring(0,7);rangeStart=selectedDay;if(b!=null){int[] saved=b.getIntArray("cart");if(saved!=null)System.arraycopy(saved,0,qty,0,Math.min(saved.length,qty.length));activePage=b.getInt("page",0);selectedDay=b.getString("selectedDay",selectedDay);selectedMonth=selectedDay.substring(0,7);rangeStart=b.getString("rangeStart",rangeStart);}draw();}
  ...
  int supplierDue(){return Math.max(0,getPreferences(0).getInt("supplier_due",getPreferences(0).getInt("supplier_due_sate",0)));}
  ...
  void drawPage(){if(activePage==1){heading("Stok & restock","Baki semalam dibawa sebagai stok awal hari ini.");
      for(int i=0;i<names.length;i++){final int id=i;if(getPreferences(0).getBoolean("menu_hidden_"+id,false))continue;if(unlimited(id)){action(names[id]+" · kuah ikut liter (tiada had unit)",()->message("Kuah kacang sentiasa boleh dijual."));continue;}
        ...
        LinearLayout controls=row();TextView start=chip(hasOpeningStock(id)?"Edit stok awal":"+ Stok awal",0xffffd54f,0xff6d3b16),more=chip("+ Restock",blue,Color.WHITE);
      ...
      LinearLayout bottom=row();metric(bottom,"DUIT KELUAR",money(cash[1]),ink,null);metric(bottom,"BAKI DUIT",money(totalMoneyBalance()),gold,null);add(body,bottom,-1,-2);gap(body,16);
      ...
      LinearLayout supplier=col();supplier.setPadding(dp(15),dp(13),dp(15),dp(13));supplier.setBackground(shape(surface,15));add(supplier,text("BAYARAN PEMBEKAL",11,muted,true),-1,-2);gap(supplier,7);add(supplier,text("Jumlah perlu dibayar",12,ink,true),-1,-2);add(supplier,text(money(supplierDue()),23,supplierDue()>0?gold:blue,true),-1,-2);gap(supplier,10);TextView supplierPaid=chip("TELAH DIBAYAR",supplierDue()>0?blue:0xff526273,Color.WHITE);supplierPaid.setGravity(Gravity.CENTER);supplierPaid.setEnabled(supplierDue()>0);supplierPaid.setAlpha(supplierDue()>0?1f:.55f);add(supplier,supplierPaid,-1,43);supplierPaid.setOnClickListener(v->paySupplier());add(body,supplier,-1,-2);gap(body,12);
      add(body,text("Masuk termasuk jualan POS • Restock item Pembekal masuk hutang dahulu",11,muted,false),-1,-2);
      ...
      action("Lihat catatan bulan dipilih",()->cashHistory());add(body,text("Baki Duit = semua jualan POS + semua duit masuk − semua duit keluar yang benar-benar telah dibayar. Hutang pembekal belum dibayar tidak ditolak.",11,muted,false),-1,-2);
  ...
  void stockEntryItem(int id,boolean opening){if(unlimited(id)){message("Kuah kacang diurus mengikut liter, tanpa had stok unit.");return;}if(opening&&hasOpeningStock(id)){editOpeningStock(id);return;}
    ...
    new AlertDialog.Builder(this).setTitle((opening?"Stok awal · ":"Restock · ")+names[id]).setMessage("Baki sebelum tambah: "+before+(opening?"\nStok awal tidak dikira sebagai belian baru.":"\nKos: "+money(costUnit(id))+" / "+costPack(id)+" unit. "+(supplierItem(id)?"Restock ini akan masuk ke Bayaran Pembekal sehingga ditanda Telah Dibayar.":"Restock akan dicatat sebagai duit keluar.")))
      .setView(input).setPositiveButton("Simpan",(dialog,w)->{try{int value=Integer.parseInt(input.getText().toString().trim());if(value<0||(!opening&&value==0)||value>100000)throw new NumberFormatException();if(opening&&hasOpeningStock(id))throw new Exception("Stok awal sudah direkod");int cost=opening?0:restockCost(id,value);JSONObject record=new JSONObject();record.put("time",timestamp());record.put("item",id);record.put("qty",value);record.put("type",opening?"opening":"restock");record.put("cost",cost);
        ...
        if(!opening){if(supplierItem(id)){int due=supplierDue();long next=(long)due+cost;if(next>Integer.MAX_VALUE)throw new Exception("Jumlah pembekal terlalu besar");editor.putInt("supplier_due",(int)next);}else{JSONObject expense=new JSONObject();expense.put("time",timestamp());expense.put("month",date().substring(0,7));expense.put("amount",-cost);expense.put("category","Restock · "+names[id]);expense.put("note",value+" unit × "+money(costUnit(id))+" / "+costPack(id)+" unit");JSONArray cash=entries("cash_entries");cash.put(expense);editor.putString("cash_entries",cash.toString());}}
  ...
  void cashHistory(){String month=selectedMonth;JSONArray list=entries("cash_entries");int incoming=0,outgoing=0;StringBuilder b=new StringBuilder();for(int i=0;i<list.length();i++){JSONObject entry=list.optJSONObject(i);if(entry==null||legacySupplierRestock(entry)||!month.equals(entry.optString("month")))continue;int value=entry.optInt("amount");if(value>0)incoming+=value;else outgoing-=value;b.append(entry.optString("time")).append(value>0?"  MASUK ":"  KELUAR ").append(money(Math.abs(value))).append("\n").append(entry.optString("category",entry.optString("note")));String note=entry.optString("note");if(!note.isEmpty()&&!note.equals(entry.optString("category")))b.append(" · ").append(note);b.append("\n\n");}
