...
googleSignInClient = GoogleSignIn.getClient(this, gso);snapshotBeforeUpdate();loadMenu();clearCurrentNasiDamageOnce();applyCorrection20260926();applySupplierReceiptDetailFix20260926();selectedDay=date();selectedMonth=selectedDay.substring(0,7);rangeStart=selectedDay;selectedStockDay=selectedDay;selectedCashDay=selectedDay;if(b!=null){int[] saved=b.getIntArray("cart");if(saved!=null)System.arraycopy(saved,0,qty,0,Math.min(saved.length,qty.length));activePage=b.getInt("page",0);selectedDay=b.getString("selectedDay",selectedDay);selectedMonth=selectedDay.substring(0,7);rangeStart=b.getString("rangeStart",rangeStart);selectedStockDay=b.getString("selectedStockDay",selectedStockDay);selectedCashDay=b.getString("selectedCashDay",selectedCashDay);cashDailyMode=b.getBoolean("cashDailyMode",cashDailyMode);}draw();}
  ...
  void stockBalance(){String day=selectedStockDay;ScrollView scroll=new ScrollView(this);LinearLayout panel=col();panel.setPadding(dp(14),dp(12),dp(14),dp(14));panel.setBackgroundColor(cream);scroll.addView(panel);LinearLayout head=col();head.setPadding(dp(14),dp(12),dp(14),dp(12));head.setBackground(shape(0xff102f4d,14));add(head,text("RINGKASAN BAKI STOK",12,gold,true),-1,-2);add(head,text(day,19,ink,true),-1,-2);add(head,text("Awal + restock − terjual − rosak = baki",11,muted,false),-1,-2);add(panel,head,-1,-2);gap(panel,10);int[] soldDay=soldBetween(day,day);for(int i=0;i<names.length;i++){if(getPreferences(0).getBoolean("menu_hidden_"+i,false))continue;LinearLayout c=col();c.setPadding(dp(13),dp(11),dp(13),dp(11));c.setBackground(shape(surface,13));LinearLayout title=row();title.addView(text(names[i],15,ink,true),new LinearLayout.LayoutParams(0,-2,1));if(unlimited(i)){add(title,chip(soldDay[i]+" TERJUAL",0xff294563,blue),-2,-2);add(c,title,-1,-2);add(c,text("Stok tanpa had unit",11,muted,false),-1,-2);}else{int[] st=stockForDay(i,day);add(title,chip("BAKI "+st[3],0xff203f61,st[3]>0?blue:0xffff6b6b),-2,-2);add(c,title,-1,-2);gap(c,8);LinearLayout stats=row();String[] lab={"AWAL","RESTOCK","TERJUAL","ROSAK"};int[] val={st[0],st[1],st[2],st[4]};for(int n=0;n<4;n++){LinearLayout box=col();box.setPadding(dp(6),dp(6),dp(6),dp(6));box.setBackground(shape(0xff20384f,8));TextView l=text(lab[n],9,muted,true);l.setGravity(Gravity.CENTER);add(box,l,-1,-2);TextView vv=text(""+val[n],15,n==3&&val[n]>0?0xffff6b6b:ink,true);vv.setGravity(Gravity.CENTER);add(box,vv,-1,-2);LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(0,-2,1);lp.setMargins(dp(2),0,dp(2),0);stats.addView(box,lp);}add(c,stats,-1,-2);}add(panel,c,-1,-2);gap(panel,8);}new AlertDialog.Builder(this).setTitle("Baki stok").setView(scroll).setPositiveButton("Tutup",null).setNeutralButton(date().equals(day)?"Tambah stok":"Pilih tarikh",(d,w)->{if(date().equals(day))stockEntry();else chooseStockDay();}).show();}
