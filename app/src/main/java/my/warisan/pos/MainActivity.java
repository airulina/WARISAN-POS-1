...
googleSignInClient = GoogleSignIn.getClient(this, gso);snapshotBeforeUpdate();loadMenu();clearCurrentNasiDamageOnce();applyCorrection20260926();applySupplierReceiptDetailFix20260926();selectedDay=date();selectedMonth=selectedDay.substring(0,7);rangeStart=selectedDay;selectedStockDay=selectedDay;selectedCashDay=selectedDay;if(b!=null){int[] saved=b.getIntArray("cart");if(saved!=null)System.arraycopy(saved,0,qty,0,Math.min(saved.length,qty.length));activePage=b.getInt("page",0);selectedDay=b.getString("selectedDay",selectedDay);selectedMonth=selectedDay.substring(0,7);rangeStart=b.getString("rangeStart",rangeStart);selectedStockDay=b.getString("selectedStockDay",selectedStockDay);selectedCashDay=b.getString("selectedCashDay",selectedCashDay);cashDailyMode=b.getBoolean("cashDailyMode",cashDailyMode);}draw();}
  ...
  void addNavigation(LinearLayout screen){LinearLayout nav=row();nav.setBackgroundColor(cream);nav.setPadding(dp(7),dp(7),dp(7),dp(9));String[] labels={"MENU","STOK","GRAF","DUIT","SETTING"};
  ...
  boolean personalCash(JSONObject e){if(e==null)return false;if(e.optBoolean("personal",false))return true;String c=e.optString("category","");return e.optInt("amount",0)<0&&(c.equals("Lain-lain")||c.equals("Nescafe / Minuman · Peribadi")||c.equals("Makan / Air kedai · Peribadi")||c.equals("Makan / Minum · Peribadi"));}
  ...
  int businessProfit(String period,boolean daily){String start=daily?period:period+"-01",end=daily?period:period+"-31";int grossSales=daily?getPreferences(0).getInt("sales_"+period,0):monthTotal(period);int netSales=Math.max(0,grossSales-refundedSalesBetween(start,end));return netSales-productCostBetween(start,end)-operatingExpenseTotal(period,daily);}
  ...
  void drawPage(){if(activePage==1){heading("Stok & restock","Pilih tarikh untuk semak rekod stok. Baki hari sebelumnya dibawa ke hari seterusnya.");
      ...
      int personal=personalTotal(cashDailyMode?selectedCashDay:pm,cashDailyMode);int profit=businessProfit(cashDailyMode?selectedCashDay:pm,cashDailyMode);
      LinearLayout top=row();metric(top,cashDailyMode?"JUALAN HARI":"JUALAN POS",money(sale),blue,null);metric(top,"UNTUNG PERNIAGAAN",money(profit),profit>=0?gold:0xffff6b6b,null);add(body,top,-1,-2);gap(body,8);
      LinearLayout flow=row();metric(flow,"DUIT MASUK",money(cash[0]),blue,null);metric(flow,"DUIT KELUAR",money(cash[1]),ink,null);add(body,flow,-1,-2);gap(body,8);
  ...
  void applyDarkReportDialog(AlertDialog dialog){
    dialog.setOnShowListener(v->{
      if(dialog.getWindow()!=null){
        dialog.getWindow().setBackgroundDrawable(shape(cream,18));
        View decor=dialog.getWindow().getDecorView();decor.setBackgroundColor(cream);
      }
      int titleId=getResources().getIdentifier("alertTitle","id","android");
      TextView title=dialog.findViewById(titleId);if(title!=null){title.setTextColor(ink);title.setTextSize(22);title.setTypeface(null,Typeface.BOLD);}
      int[] buttons={AlertDialog.BUTTON_POSITIVE,AlertDialog.BUTTON_NEGATIVE,AlertDialog.BUTTON_NEUTRAL};
      for(int id:buttons){Button b=dialog.getButton(id);if(b!=null){b.setTextColor(gold);b.setTypeface(null,Typeface.BOLD);b.setBackgroundColor(cream);}}
    });
    dialog.show();
  }
  void stockBalance(){String day=selectedStockDay;ScrollView scroll=new ScrollView(this);scroll.setVerticalScrollBarEnabled(false);LinearLayout panel=col();panel.setPadding(dp(16),dp(16),dp(16),dp(16));panel.setBackgroundColor(cream);scroll.addView(panel);
    ...
    AlertDialog dialog=new AlertDialog.Builder(this).setView(scroll).setPositiveButton("TUTUP",null).setNeutralButton(date().equals(day)?"TAMBAH STOK":"PILIH TARIKH",(d,w)->{if(date().equals(day))stockEntry();else chooseStockDay();}).create();applyDarkReportDialog(dialog);}
  ...
  void cashEntry(boolean incoming){
    ...
    EditText amount=new EditText(this);amount.setSingleLine(true);amount.setInputType(android.text.InputType.TYPE_CLASS_NUMBER|android.text.InputType.TYPE_NUMBER_FLAG_DECIMAL);amount.setHint("Contoh: 10.50");amount.setTextColor(ink);amount.setHintTextColor(muted);amount.setBackgroundTintList(android.content.res.ColorStateList.valueOf(blue));add(form,amount,-1,52);gap(form,10);
    ...
    add(form,text("CATATAN",11,muted,true),-1,-2);EditText note=new EditText(this);note.setSingleLine(true);note.setHint("Catatan tambahan (jika ada)");note.setTextColor(ink);note.setHintTextColor(muted);note.setBackgroundTintList(android.content.res.ColorStateList.valueOf(blue));add(form,note,-1,50);
    if(!incoming){gap(form,10);TextView info=text("Nota: Lain-lain dikira sebagai PENGELUARAN PERIBADI.",11,gold,true);info.setPadding(dp(10),dp(8),dp(10),dp(8));info.setBackground(shape(0xff293a4d,9));add(form,info,-1,-2);}
    AlertDialog dialog=new AlertDialog.Builder(this).setTitle(incoming?"Catat duit masuk":"Catat duit keluar").setView(form).setPositiveButton("SIMPAN",null).setNegativeButton("BATAL",null).create();
    dialog.setOnShowListener(v->{if(dialog.getWindow()!=null)dialog.getWindow().setBackgroundDrawable(shape(cream,18));int titleId=getResources().getIdentifier("alertTitle","id","android");TextView title=dialog.findViewById(titleId);if(title!=null){title.setTextColor(ink);title.setTypeface(null,Typeface.BOLD);}dialog.getButton(AlertDialog.BUTTON_NEGATIVE).setTextColor(muted);Button save=dialog.getButton(AlertDialog.BUTTON_POSITIVE);save.setTextColor(gold);save.setTypeface(null,Typeface.BOLD);save.setOnClickListener(x->{try{java.math.BigDecimal rm=new java.math.BigDecimal(amount.getText().toString().trim());int cents=rm.movePointRight(2).intValueExact();if(cents<=0)throw new Exception();JSONObject entry=new JSONObject();entry.put("time",timestamp());entry.put("month",date().substring(0,7));entry.put("amount",incoming?cents:-cents);String chosen=categories[category.getSelectedItemPosition()];entry.put("category",chosen);entry.put("personal",!incoming&&(chosen.contains("Peribadi")||"Lain-lain".equals(chosen)));entry.put("note",note.getText().toString().trim());appendEntry("cash_entries",entry);dialog.dismiss();draw();message("Catatan disimpan");}catch(Exception e){message("Masukkan jumlah RM yang sah");}});});dialog.show();}
  ...
  void saleHistory(String day){
    ...
    AlertDialog dialog=new AlertDialog.Builder(this).setView(scroll).setPositiveButton("TUTUP",null).setNeutralButton("TUKAR TARIKH",(d,w)->saleHistoryChooseDate()).create();applyDarkReportDialog(dialog);
  }
  ...
  void refundSale(int saleIndex){
    try{
      ...
      JSONArray cash=entries("cash_entries");JSONObject out=new JSONObject();out.put("time",now);out.put("month",date().substring(0,7));out.put("amount",-refund);out.put("category","Refund Customer");out.put("note","Refund transaksi "+sale.optString("time"));cash.put(out);
