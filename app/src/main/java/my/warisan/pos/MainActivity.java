...
googleSignInClient = GoogleSignIn.getClient(this, gso);if(!getPreferences(0).contains("supplier_due_sate"))getPreferences(0).edit().putInt("supplier_due_sate",17500).commit();snapshotBeforeUpdate();loadMenu();selectedDay=date();selectedMonth=selectedDay.substring(0,7);rangeStart=selectedDay;if(b!=null){int[] saved=b.getIntArray("cart");if(saved!=null)System.arraycopy(saved,0,qty,0,Math.min(saved.length,qty.length));activePage=b.getInt("page",0);selectedDay=b.getString("selectedDay",selectedDay);selectedMonth=selectedDay.substring(0,7);rangeStart=b.getString("rangeStart",rangeStart);}draw();}
  ...
  int supplierDue(){return Math.max(0,getPreferences(0).getInt("supplier_due_sate",0));}
  boolean sateItem(int id){return id>=0&&id<3;}
  void paySupplier(){int due=supplierDue();if(due<=0){message("Tiada bayaran pembekal sate tertunggak.");return;}new AlertDialog.Builder(this).setTitle("Bayar pembekal sate?").setMessage("Jumlah perlu dibayar: "+money(due)+"\n\nSelepas disahkan, jumlah ini akan direkod sebagai Duit Keluar.").setPositiveButton("TELAH DIBAYAR",(d,w)->{try{JSONObject expense=new JSONObject();expense.put("time",timestamp());expense.put("month",date().substring(0,7));expense.put("amount",-due);expense.put("category","Bayaran Pembekal Sate");expense.put("note","Bayaran hutang restock sate");JSONArray cash=entries("cash_entries");cash.put(expense);android.content.SharedPreferences.Editor editor=getPreferences(0).edit().putString("cash_entries",cash.toString()).putInt("supplier_due_sate",0);if(!editor.commit())throw new Exception();draw();message("Bayaran pembekal "+money(due)+" telah direkod.");}catch(Exception e){message("Gagal merekod bayaran pembekal.");}}).setNegativeButton("Batal",null).show();}
  void drawPage(){if(activePage==1){heading("Stok & restock","Baki semalam dibawa sebagai stok awal hari ini.");
      for(int i=0;i<names.length;i++){final int id=i;if(getPreferences(0).getBoolean("menu_hidden_"+id,false))continue;if(unlimited(id)){action(names[id]+" · kuah ikut liter (tiada had unit)",()->message("Kuah kacang sentiasa boleh dijual."));continue;}
        ...
        LinearLayout controls=row();TextView start=chip(hasOpeningStock(id)?"Awal ✓":"+ Stok awal",0xffffd54f,0xff6d3b16),more=chip("+ Restock",blue,Color.WHITE);
      ...
      LinearLayout supplier=col();supplier.setPadding(dp(15),dp(13),dp(15),dp(13));supplier.setBackground(shape(surface,15));add(supplier,text("BAYARAN PEMBEKAL SATE",11,muted,true),-1,-2);gap(supplier,7);add(supplier,text("Jumlah perlu dibayar",12,ink,true),-1,-2);add(supplier,text(money(supplierDue()),23,supplierDue()>0?gold:blue,true),-1,-2);gap(supplier,10);TextView supplierPaid=chip("TELAH DIBAYAR",supplierDue()>0?blue:0xff526273,Color.WHITE);supplierPaid.setGravity(Gravity.CENTER);supplierPaid.setEnabled(supplierDue()>0);supplierPaid.setAlpha(supplierDue()>0?1f:.55f);add(supplier,supplierPaid,-1,43);supplierPaid.setOnClickListener(v->paySupplier());add(body,supplier,-1,-2);gap(body,12);
      add(body,text("Masuk termasuk jualan POS • Restock sate masuk Bayaran Pembekal dahulu",11,muted,false),-1,-2);
  ...
  void stockEntryItem(int id,boolean opening){if(unlimited(id)){message("Kuah kacang diurus mengikut liter, tanpa had stok unit.");return;}if(opening&&hasOpeningStock(id)){message("Stok awal sudah direkod. Gunakan Restock untuk tambah.");return;}
    ...
    new AlertDialog.Builder(this).setTitle((opening?"Stok awal · ":"Restock · ")+names[id]).setMessage("Baki sebelum tambah: "+before+(opening?"\nStok awal tidak dikira sebagai belian baru.":"\nKos: "+money(costUnit(id))+" / "+costPack(id)+" unit. "+(sateItem(id)?"Restock sate akan masuk ke Bayaran Pembekal sehingga ditanda Telah Dibayar.":"Restock akan dicatat sebagai duit keluar.")))
