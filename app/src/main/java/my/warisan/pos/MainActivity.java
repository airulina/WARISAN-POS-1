...
public class MainActivity extends Activity {
  ...
  void confirm(String method,int due){new AlertDialog.Builder(this).setTitle("Sahkan bayaran").setMessage(method+" · "+money(due)+"\n\nPastikan bayaran sudah diterima sebelum simpan.")
    .setPositiveButton("Bayaran diterima",(d,w)->{
      ...
      previewReceipt(printed);
    }).setNegativeButton("Kembali",null).show();}
  void previewReceipt(String printed){
    ScrollView scroll=new ScrollView(this);
    LinearLayout sheet=col();sheet.setPadding(dp(16),dp(12),dp(16),dp(12));sheet.setBackgroundColor(Color.WHITE);scroll.addView(sheet);
    ...
    LinearLayout.LayoutParams lp=params(-1,95);sheet.addView(logo,lp);
    TextView details=text(printed,13,ink,false);details.setTypeface(Typeface.MONOSPACE);details.setTextIsSelectable(true);add(sheet,details,-1,-2);
    new AlertDialog.Builder(this).setTitle("Resit bayaran · Semak dahulu").setView(scroll)
  ...
  void print(String receipt){
    ...
    new Thread(()->{
      ...
      try{
        ...
        out.write(new byte[]{27,64});printLogo(out);out.write(receipt.getBytes(StandardCharsets.US_ASCII));out.write(new byte[]{10,10,10});out.flush();
