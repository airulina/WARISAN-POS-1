...
private static final int RC_SIGN_IN = 9001;
  ...
  boolean hasOpeningStock(int id){JSONArray list=entries("stock_entries");for(int i=0;i<list.length();i++){JSONObject record=list.optJSONObject(i);if(record!=null&&record.optInt("item",-1)==id){String type=record.optString("type","");if("opening".equals(type)||"opening_edit".equals(type)||!record.has("type"))return true;}}return false;}
  int[] stockToday(int id){int[] summary=new int[5];String today=date();JSONArray incoming=entries("stock_entries"),sales=entries("stock_sales");boolean firstLegacy=true;
    for(int i=0;i<incoming.length();i++){JSONObject e=incoming.optJSONObject(i);if(e==null||e.optInt("item",-1)!=id)continue;String day=entryDay(e);int value=e.optInt("qty");boolean opening="opening".equals(e.optString("type"))||"opening_edit".equals(e.optString("type"))||(firstLegacy&&!e.has("type"));firstLegacy=false;
...
googleSignInClient = GoogleSignIn.getClient(this, gso);snapshotBeforeUpdate();loadMenu();selectedDay=date();selectedMonth=selectedDay.substring(0,7);rangeStart=selectedDay;if(b!=null){int[] saved=b.getIntArray("cart");if(saved!=null)System.arraycopy(saved,0,qty,0,Math.min(saved.length,qty.length));activePage=b.getInt("page",0);selectedDay=b.getString("selectedDay",selectedDay);selectedMonth=selectedDay.substring(0,7);rangeStart=b.getString("rangeStart",rangeStart);}draw();}
  ...
  JSONObject customMenuItem(int id){if(id<7)return null;return entries("custom_menu").optJSONObject(id-7);}
  boolean supplierItem(int id){if(sateItem(id)||id==5||id==6)return true;JSONObject item=customMenuItem(id);return item!=null&&item.optBoolean("supplier",false);}
  String supplierKey(int id){return sateItem(id)?"sate":(id==5?"laksa":(id==6?"kuih":"menu_"+id));}
  String supplierLabel(String key){if("laksa".equals(key))return "Laksa";if("kuih".equals(key))return "Kuih Siput";if("sate".equals(key))return "Sate";if(key!=null&&key.startsWith("menu_")){try{int id=Integer.parseInt(key.substring(5));if(id>=0&&id<names.length)return names[id];}catch(Exception ignored){}}return "Pembekal";}
  int supplierDue(String key){return Math.max(0,getPreferences(0).getInt("supplier_due_"+key,0));}
  int supplierDue(){return supplierDue("sate");}
  boolean deferredSateRestock(JSONObject e){String c=e==null?"":e.optString("category","");return c.startsWith("Restock · Sate Ayam")||c.startsWith("Restock · Sate Daging")||c.startsWith("Restock · Sate Kambing")||c.startsWith("Restock · Laksa Utara")||c.startsWith("Restock · Kuih Siput");}
  int[] cashTotals(String month){int[] totals=new int[2];JSONArray entries=entries("cash_entries");for(int i=0;i<entries.length();i++){JSONObject e=entries.optJSONObject(i);if(e==null||!month.equals(e.optString("month"))||deferredSateRestock(e))continue;int amount=e.optInt("amount");if(amount>=0)totals[0]+=amount;else totals[1]-=amount;}return totals;}
  int rmCents(EditText input){try{java.math.BigDecimal rm=new java.math.BigDecimal(input.getText().toString().trim().isEmpty()?"0":input.getText().toString().trim());return rm.movePointRight(2).intValueExact();}catch(Exception e){return -1;}}
  void paySupplier(){paySupplier("sate");}
  void paySupplier(final String key){final int due=supplierDue(key);final String label=supplierLabel(key);if(due<=0){message("Tiada bayaran pembekal "+label.toLowerCase(Locale.US)+" tertunggak.");return;}
    LinearLayout form=col();form.setPadding(dp(18),dp(6),dp(18),0);add(form,text("Jumlah tertunggak  "+money(due),14,ink,true),-1,-2);gap(form,10);
    EditText paid=new EditText(this);paid.setSingleLine(true);paid.setInputType(android.text.InputType.TYPE_CLASS_NUMBER|android.text.InputType.TYPE_NUMBER_FLAG_DECIMAL);paid.setHint("Jumlah dibayar");paid.setText(String.format(Locale.US,"%.2f",due/100.0));add(form,paid,-1,-2);gap(form,8);
    EditText discount=new EditText(this);discount.setSingleLine(true);discount.setInputType(android.text.InputType.TYPE_CLASS_NUMBER|android.text.InputType.TYPE_NUMBER_FLAG_DECIMAL);discount.setHint("Tolakan / diskaun · contoh 5.00");discount.setText("0.00");add(form,discount,-1,-2);gap(form,8);
    TextView preview=text("Bayaran + tolakan mesti tidak melebihi jumlah tertunggak.",12,muted,true);add(form,preview,-1,-2);
    AlertDialog dialog=new AlertDialog.Builder(this).setTitle("Bayaran pembekal "+label).setView(form).setPositiveButton("CONFIRM",null).setNegativeButton("Batal",null).create();
    android.text.TextWatcher watcher=new android.text.TextWatcher(){public void beforeTextChanged(CharSequence s,int a,int b,int c){}public void afterTextChanged(android.text.Editable e){}public void onTextChanged(CharSequence s,int a,int b,int c){int p=rmCents(paid),d=rmCents(discount);if(p<0||d<0||p+d>due)preview.setText("Semak jumlah dibayar / tolakan.");else preview.setText("Duit keluar: "+money(p)+"  •  Tolakan: "+money(d)+"  •  Baki: "+money(due-p-d));}};
    paid.addTextChangedListener(watcher);discount.addTextChangedListener(watcher);
    dialog.setOnShowListener(v->dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(view->{int amount=rmCents(paid),cut=rmCents(discount);if(amount<0||cut<0||amount+cut<=0||amount+cut>due){message("Semak jumlah bayaran dan tolakan");return;}int remain=due-amount-cut;try{JSONArray cash=entries("cash_entries");if(amount>0){JSONObject expense=new JSONObject();expense.put("time",timestamp());expense.put("month",date().substring(0,7));expense.put("amount",-amount);expense.put("category","Bayaran Pembekal "+label);expense.put("note","Hutang "+money(due)+" · Tolakan "+money(cut)+" · Baki "+money(remain));cash.put(expense);}int no=getPreferences(0).getInt("supplier_receipt_no",0)+1;android.content.SharedPreferences.Editor editor=getPreferences(0).edit().putString("cash_entries",cash.toString()).putInt("supplier_due_"+key,remain).putInt("supplier_receipt_no",no);if(!editor.commit())throw new Exception();dialog.dismiss();draw();previewSupplierReceipt(label,due,amount,cut,remain,no,false);}catch(Exception e){message("Gagal merekod bayaran pembekal.");}}));dialog.show();}
  String supplierReceipt(String label,int original,int paid,int discount,int remain,int no){StringBuilder b=new StringBuilder();b.append(centerReceipt(getPreferences(0).getString("shop_name","WARISAN FROZEN"))).append("--------------------------------\n").append(line("RESIT PEMBEKAL",String.format(Locale.US,"#P%04d",no))).append(line("Kategori",label)).append(line("Tarikh",new SimpleDateFormat("dd/MM/yy HH:mm",Locale.US).format(new Date()))).append("--------------------------------\n").append(line("HUTANG ASAL",money(original))).append(line("TOLAKAN",money(discount))).append(line("DIBAYAR",money(paid))).append(line("BAKI",money(remain))).append("================================\n").append(ownerReceipt()).append("\n");return b.toString();}
  void previewSupplierReceipt(String label,int original,int paid,int discount,int remain,int no,boolean sample){String printed=supplierReceipt(label,original,paid,discount,remain,no);ScrollView scroll=new ScrollView(this);LinearLayout sheet=col();sheet.setPadding(dp(18),dp(14),dp(18),dp(14));sheet.setBackgroundColor(Color.WHITE);scroll.addView(sheet);int receiptMuted=0xff66788a;ImageView logo=new ImageView(this);logo.setImageResource(R.drawable.warisan_logo);makeCircle(logo);LinearLayout logoRow=row();logoRow.setGravity(Gravity.CENTER);add(logoRow,logo,70,70);add(sheet,logoRow,-1,-2);gap(sheet,5);TextView brand=text(getPreferences(0).getString("shop_name","WARISAN FROZEN"),17,blue,true);brand.setGravity(Gravity.CENTER);add(sheet,brand,-1,-2);receiptRule(sheet);receiptRow(sheet,"RESIT PEMBEKAL",String.format(Locale.US,"#P%04d",no),false);receiptRow(sheet,"Kategori",label,false);receiptRow(sheet,"Tarikh",new SimpleDateFormat("dd/MM/yyyy HH:mm",Locale.US).format(new Date()),false);receiptRule(sheet);receiptRow(sheet,"Jumlah hutang",money(original),false);receiptRow(sheet,"Tolakan / diskaun",money(discount),false);receiptRow(sheet,"JUMLAH DIBAYAR",money(paid),true);receiptRow(sheet,"Baki hutang",money(remain),false);receiptRule(sheet);String owner=ownerDisplay();if(!owner.isEmpty()){TextView owned=text("DIMILIKI OLEH : "+owner,10,receiptMuted,true);owned.setGravity(Gravity.CENTER);add(sheet,owned,-1,-2);}new AlertDialog.Builder(this).setTitle(sample?"Preview resit pembekal":"Bayaran pembekal direkod").setView(scroll).setPositiveButton("Cetak resit",(d,w)->print(printed)).setNegativeButton("Tutup",null).show();}
  void previewTestSupplierReceipt(){previewSupplierReceipt("Sate",17500,17000,500,0,1,true);}
  void supplierCard(String key){String label=supplierLabel(key);int due=supplierDue(key);LinearLayout supplier=col();supplier.setPadding(dp(15),dp(13),dp(15),dp(13));supplier.setBackground(shape(surface,15));add(supplier,text("BAYARAN PEMBEKAL "+label.toUpperCase(Locale.US),11,muted,true),-1,-2);gap(supplier,7);add(supplier,text("Jumlah perlu dibayar",12,ink,true),-1,-2);add(supplier,text(money(due),23,due>0?gold:blue,true),-1,-2);gap(supplier,10);TextView paid=chip("TELAH DIBAYAR",due>0?blue:0xff526273,Color.WHITE);paid.setGravity(Gravity.CENTER);paid.setEnabled(due>0);paid.setAlpha(due>0?1f:.55f);add(supplier,paid,-1,43);paid.setOnClickListener(v->paySupplier(key));add(body,supplier,-1,-2);gap(body,12);}
  void drawPage(){if(activePage==1){heading("Stok & restock","Baki semalam dibawa sebagai stok awal hari ini.");
      for(int i=0;i<names.length;i++){final int id=i;if(getPreferences(0).getBoolean("menu_hidden_"+id,false))continue;if(unlimited(id)){action(names[id]+" · kuah ikut liter (tiada had unit)",()->message("Kuah kacang sentiasa boleh dijual."));continue;}
        ...
        LinearLayout controls=row();TextView start=chip("Edit stok awal",0xffffd54f,0xff6d3b16),more=chip("+ Restock",blue,Color.WHITE);
        ...
        start.setOnClickListener(v->editOpeningStock(id));more.setOnClickListener(v->stockEntryItem(id,false));add(card,controls,-1,-2);gap(card,7);TextView damage=chip("− Stok rosak",0xffff2d2d,Color.WHITE);add(card,damage,-1,40);damage.setOnClickListener(v->damageEntry(id));
      ...
      supplierCard("sate");supplierCard("laksa");supplierCard("kuih");
      for(int sid=7;sid<names.length;sid++)if(supplierItem(sid))supplierCard(supplierKey(sid));
      add(body,text("Menu yang ditetapkan sebagai PEMBEKAL akan jadi hutang bila restock. Hutang hanya masuk Duit Keluar selepas bayaran disahkan.",11,muted,false),-1,-2);
      ...
      action("Lihat catatan bulan dipilih",()->cashHistory());add(body,text("Baki bulan = jualan POS + duit masuk − duit keluar. Restock menu berstatus PEMBEKAL menjadi hutang dahulu dan hanya jadi Duit Keluar selepas bayaran disahkan. Menu SENDIRI tidak menambah hutang pembekal; kos restock terus direkod sebagai Duit Keluar. Belanja lain direkod melalui − CATAT KELUAR. Stok rosak ialah kerugian nilai stok. Baki bulan bukan untung bersih.",11,muted,false),-1,-2);
    }else if(activePage==4){
      heading("Setting","WarisanPOS 3.18 · Tetapan kedai dan data.");
      ...
      action("Preview resit pembekal",()->previewTestSupplierReceipt());
  ...
  void editOpeningStock(int id){if(unlimited(id)){message("Kuah kacang diurus mengikut liter.");return;}int[] now=stockToday(id);int oldOpening=now[0];int minOpening=Math.max(0,now[2]+now[4]-now[1]);EditText input=new EditText(this);input.setSingleLine(true);input.setInputType(android.text.InputType.TYPE_CLASS_NUMBER);input.setText(""+oldOpening);input.setSelectAllOnFocus(true);new AlertDialog.Builder(this).setTitle("Edit stok awal · "+names[id]).setMessage("Stok awal sekarang: "+oldOpening+"\nMinimum: "+minOpening+" supaya baki tidak jadi negatif.\n\nEdit stok awal tidak menambah kos atau hutang pembekal.").setView(input).setPositiveButton("Simpan",(d,w)->{try{int target=Integer.parseInt(input.getText().toString().trim());if(target<minOpening||target>100000)throw new Exception();int delta=target-oldOpening;if(delta!=0){JSONObject record=new JSONObject();record.put("time",timestamp());record.put("item",id);record.put("qty",delta);record.put("type","opening_edit");record.put("cost",0);JSONArray stocks=entries("stock_entries");stocks.put(record);if(!getPreferences(0).edit().putString("stock_entries",stocks.toString()).commit())throw new Exception();}draw();message("Stok awal "+names[id]+" ditukar kepada "+target);}catch(Exception e){message("Semak jumlah stok awal");}}).setNegativeButton("Batal",null).show();}
  void stockEntryItem(int id,boolean opening){if(opening){editOpeningStock(id);return;}if(unlimited(id)){message("Kuah kacang diurus mengikut liter, tanpa had stok unit.");return;}if(costUnit(id)<=0){new AlertDialog.Builder(this).setTitle("Harga mentah belum ditetapkan").setMessage("Tetapkan harga mentah "+names[id]+" sebelum restock.").setPositiveButton("Edit harga",(d,w)->editMenuPrice(id)).setNegativeButton("Batal",null).show();return;}
    LinearLayout form=col();form.setPadding(dp(18),dp(5),dp(18),0);EditText input=new EditText(this);input.setInputType(android.text.InputType.TYPE_CLASS_NUMBER);input.setHint("Bilangan unit / cucuk");add(form,input,-1,-2);gap(form,8);CheckBox free=new CheckBox(this);free.setText("Tambah tanpa kos / tanpa hutang pembekal");add(form,free,-1,-2);TextView hint=text("Guna pilihan ini jika stok diterima percuma, contoh pembekal bayar hutang kepada anda menggunakan sate.",11,muted,false);add(form,hint,-1,-2);int before=stock(id);
    new AlertDialog.Builder(this).setTitle("Restock · "+names[id]).setMessage("Baki sebelum tambah: "+before+"\nKos biasa: "+money(costUnit(id))+" / "+costPack(id)+" unit.").setView(form).setPositiveButton("Simpan",(dialog,w)->{try{int value=Integer.parseInt(input.getText().toString().trim());if(value<=0||value>100000)throw new NumberFormatException();boolean noCost=free.isChecked();int cost=noCost?0:restockCost(id,value);JSONObject record=new JSONObject();record.put("time",timestamp());record.put("item",id);record.put("qty",value);record.put("type",noCost?"restock_free":"restock");record.put("cost",cost);JSONArray stocks=entries("stock_entries");stocks.put(record);android.content.SharedPreferences.Editor editor=getPreferences(0).edit().putString("stock_entries",stocks.toString());
      if(!noCost&&supplierItem(id)){String key=supplierKey(id);int due=supplierDue(key);long next=(long)due+cost;if(next>Integer.MAX_VALUE)throw new Exception("Jumlah pembekal terlalu besar");editor.putInt("supplier_due_"+key,(int)next);}
      else if(!noCost&&cost>0){JSONArray cash=entries("cash_entries");JSONObject expense=new JSONObject();expense.put("time",timestamp());expense.put("month",date().substring(0,7));expense.put("amount",-cost);expense.put("category","Kos Restock · "+names[id]);expense.put("note","Menu SENDIRI · "+value+" unit");cash.put(expense);editor.putString("cash_entries",cash.toString());}
      if(!editor.commit())throw new Exception("Gagal simpan rekod");draw();String costText=noCost?"\nTanpa kos / tanpa hutang pembekal":(supplierItem(id)?"\nBayaran pembekal bertambah: "+money(cost)+"\nJumlah belum dibayar: "+money(supplierDue(supplierKey(id))):"\nKos restock terus direkod sebagai Duit Keluar: "+money(cost));new AlertDialog.Builder(this).setTitle("Stok berjaya dikemas kini").setMessage("Baki "+before+" + restock "+value+" = "+stock(id)+"\n\n"+names[id]+costText).setPositiveButton("OK",null).show();}catch(Exception e){message("Gagal simpan. Semak bilangan stok dan cuba lagi.");}}).setNegativeButton("Batal",null).show();}
  ...
  void addMenu(){LinearLayout form=col();form.setPadding(dp(18),dp(5),dp(18),0);EditText name=new EditText(this),price=new EditText(this);name.setSingleLine(true);name.setHint("Nama menu");price.setInputType(android.text.InputType.TYPE_CLASS_NUMBER|android.text.InputType.TYPE_NUMBER_FLAG_DECIMAL);price.setHint("Harga RM, contoh 7.00");add(form,name,-1,-2);add(form,price,-1,-2);gap(form,8);add(form,text("Sumber stok",13,ink,true),-1,-2);android.widget.RadioGroup source=new android.widget.RadioGroup(this);source.setOrientation(android.widget.RadioGroup.VERTICAL);android.widget.RadioButton supplier=new android.widget.RadioButton(this),own=new android.widget.RadioButton(this);supplier.setText("Pembekal · restock jadi hutang pembekal");own.setText("Sendiri · kos restock terus masuk Duit Keluar");source.addView(supplier);source.addView(own);supplier.setChecked(true);add(form,source,-1,-2);add(form,text("Jika pilih Pembekal, setiap restock berbayar akan muncul di bahagian DUIT sebagai jumlah perlu dibayar.",11,muted,false),-1,-2);
    new AlertDialog.Builder(this).setTitle("+ Tambah menu").setView(form).setPositiveButton("Simpan",(d,w)->{try{String title=name.getText().toString().trim();if(title.isEmpty()||title.length()>35)throw new Exception();int cents=new java.math.BigDecimal(price.getText().toString().trim()).movePointRight(2).intValueExact();if(cents<=0||cents>10000000)throw new Exception();JSONObject item=new JSONObject();item.put("name",title);item.put("price",cents);item.put("supplier",supplier.isChecked());appendEntry("custom_menu",item);names=Arrays.copyOf(names,names.length+1);names[names.length-1]=title;icons=Arrays.copyOf(icons,icons.length+1);icons[icons.length-1]="🍽";prices=Arrays.copyOf(prices,prices.length+1);prices[prices.length-1]=cents;qty=Arrays.copyOf(qty,qty.length+1);draw();message("Menu baru disimpan sebagai "+(supplier.isChecked()?"PEMBEKAL. Restock berbayar akan jadi hutang.":"SENDIRI. Kos restock akan terus masuk Duit Keluar."));}catch(Exception e){message("Semak nama dan harga menu");}}).setNegativeButton("Batal",null).show();}
  void deleteMenu(){
  ...
  void cashEntry(boolean incoming){LinearLayout form=col();form.setPadding(dp(20),dp(5),dp(20),0);EditText amount=new EditText(this);amount.setInputType(android.text.InputType.TYPE_CLASS_NUMBER|android.text.InputType.TYPE_NUMBER_FLAG_DECIMAL);amount.setHint("Jumlah RM, contoh 10.50");add(form,amount,-1,-2);
    String[] categories=incoming?new String[]{"Modal tambahan","Lain-lain"}:new String[]{"Sewa","Minyak kereta","Arang","Barang plastik / packaging","Bahan mentah lain","Utiliti","Upah / gaji","Repair / maintenance","Pengangkutan","Lain-lain"};
  ...
  void cashHistory(){String month=selectedMonth;JSONArray list=entries("cash_entries");int incoming=0,outgoing=0;StringBuilder b=new StringBuilder();for(int i=0;i<list.length();i++){JSONObject entry=list.optJSONObject(i);if(entry==null||!month.equals(entry.optString("month"))||deferredSateRestock(entry))continue;int value=entry.optInt("amount");if(value>0)incoming+=value;else outgoing-=value;b.append(entry.optString("time")).append(value>0?"  MASUK ":"  KELUAR ").append(money(Math.abs(value))).append("\n").append(entry.optString("category",entry.optString("note")));String note=entry.optString("note");if(!note.isEmpty()&&!note.equals(entry.optString("category")))b.append(" · ").append(note);b.append("\n\n");}
  ...
  int[][] cashSeries(String month){int days=daysInMonth(month);int[][] values=new int[3][days];for(int d=0;d<days;d++)values[0][d]=getPreferences(0).getInt("sales_"+month+String.format(Locale.US,"-%02d",d+1),0);
    for(String key:new String[]{"cash_entries","stock_damage"}){JSONArray list=entries(key);for(int i=0;i<list.length();i++){JSONObject e=list.optJSONObject(i);if(e==null||("cash_entries".equals(key)&&deferredSateRestock(e)))continue;String day=entryDay(e);if(!day.startsWith(month+"-"))continue;try{int d=Integer.parseInt(day.substring(8,10))-1;if(d<0||d>=days)continue;if(key.equals("stock_damage"))values[2][d]+=e.optInt("cost");else{int n=e.optInt("amount");values[n>=0?0:1][d]+=Math.abs(n);}}catch(Exception ignored){}}}return values;}
